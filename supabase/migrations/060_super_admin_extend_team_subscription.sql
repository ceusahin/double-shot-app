-- Super admin: extend team's subscription_ends_at by hours (positive or negative).
...
begin
  ...
  update public.teams set subscription_ends_at = v_next where id = p_team_id;

  return v_next;
end;
$$;

comment on function public.super_admin_extend_team_subscription(uuid, int) is
  'Super admin: adds +/- hours to team.subscription_ends_at; returns new value.';
